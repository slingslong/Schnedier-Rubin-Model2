...
The model starts with a number of fully connected scientists. HEG members are represented by circles and HUG members are represented by cubes.

At each step, a new scientist is added.  The probability that the new node is a HUG member is determined by a logistic growth equation; as time goes on, the probability approaches the population proportion of the HUG. A new node picks existing nodes to connect to randomly, but with some bias.  More specifically, a node's chance of being selected is directly proportional to the number of connections it already has, or its "degree," and its similarity to the new node.

When the model contains more than 50 nodes, each new node has a 50% chance of connecting with all link-neighbors of the first node with which it chooses to connect . This represents a scientific field which becomes established and adopts practices to train new scientists. Furthermore, beyond a specified size old nodes "retire," along with all of their links. 

We use attribution contests to model an instance of simultaneous discovery. An attribution contest works like this: One scientist in each group is randomly chosen; the HUG member is blue and the HEG member is green. At each step, a randomly chosen node turns either blue or green depending on whether the closest non-red node is blue or green. This represents each scientists learning about a discovery from someone in the network who already has a belief about who has made the discovery. A group wins when a supermajority of nodes believe that the member of said group made the discovery. An attribution contest can occur at any time-step.
...
The HOMOPHILY input determines how strongly new nodes consider similarity between themselves and target nodes when deciding their link-neighbors.
